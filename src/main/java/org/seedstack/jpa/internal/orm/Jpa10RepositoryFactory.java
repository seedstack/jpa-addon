...
public class Jpa10RepositoryFactory extends BaseJpaRepositoryFactory {
    ...
    /**
     * JPA 1.0 compliant implementation of business framework repository. It has limited support for
     * specifications: only {@link TrueSpecification}, {@link FalseSpecification} and {@link
     * IdentitySpecification} are supported. They respectively execute the operation on all
     * aggregates, no aggregate and one specific aggregate.
     *
     * @param <A> the aggregate root class.
     * @param <I> the aggregate root identifier class.
     */
    public static class Jpa10Repository<A extends AggregateRoot<I>, I> extends BaseRepository<A, I> {
        ...
        protected Jpa10Repository(Class<A> aggregateRootClass, Class<I> idClass) {
        ...
        public void add(A aggregate) throws AggregateExistsException {
            try {
                ...
                throw new AggregateExistsException(
                        "Aggregate " + getAggregateRootClass() + " identified with " + aggregate.getId()
                                + " is already persisted", e);
            }
        }

        @Override
        public Stream<A> get(Specification<A> specification,
                Option... options) {
            if (specification instanceof TrueSpecification) {
                return buildStream(applyOffsetAndLimit(entityManager.createQuery(
                        applySort(String.format(SELECT_QUERY, getAggregateRootClass().getCanonicalName()),
                                options)), options));
            } else if (specification instanceof FalseSpecification) {
                return Stream.empty();
            } else if (specification instanceof IdentitySpecification) {
                A found = entityManager.find(getAggregateRootClass(),
                        ((IdentitySpecification) specification).getExpectedIdentifier());
                if (found != null) {
                ...
                throw new UnsupportedOperationException(
                        "Only TrueSpecification (get all), FalseSpecification (get none) or "
                                + "IdentitySpecification (get one) is supported with JPA 1.0");
            }
        }
        ...
        public long count(Specification<A> specification) {
            if (specification instanceof TrueSpecification) {
                return (Long) entityManager
                        .createQuery(String.format(COUNT_QUERY, getAggregateRootClass().getCanonicalName()))
                        .getSingleResult();
            } else if (specification instanceof FalseSpecification) {
                return 0L;
            } else if (specification instanceof IdentitySpecification) {
                ...
                throw new UnsupportedOperationException(
                        "Only TrueSpecification (count all), FalseSpecification (count none) or "
                                + "IdentitySpecification (count one) is supported with JPA 1.0");
            }
        }
        ...
        public long remove(Specification<A> specification) {
            if (specification instanceof TrueSpecification) {
                return entityManager
                        .createQuery(String.format(DELETE_QUERY, getAggregateRootClass().getCanonicalName()))
                        .executeUpdate();
            } else if (specification instanceof FalseSpecification) {
                return 0L;
            } else if (specification instanceof IdentitySpecification) {
                try {
                    entityManager.remove(entityManager.getReference(getAggregateRootClass(),
                            ((IdentitySpecification) specification).getExpectedIdentifier()));
                    return 1L;
                ...
                throw new UnsupportedOperationException(
                        "Only TrueSpecification (remove all), FalseSpecification (remove none) or "
                                + "IdentitySpecification (remove one) is supported with JPA 1.0");
            }
        }
        ...
        public void remove(I id) throws AggregateNotFoundException {
            try {
                ...
                throw new AggregateNotFoundException(
                        "Non-existent aggregate " + getAggregateRootClass() + " identified with " + id
                                + " cannot be removed", e);
            }
        }
        ...
        private String applySort(String jpql, Option... options) {
            for (Option option : options) {
                if (option instanceof SortOption) {
                    ...
                    List<SortOption.SortedAttribute> sortedAttributes = ((SortOption) option)
                            .getSortedAttributes();
